console.log("LOADED js/script.js", location.href);

'use strict';
console.clear();

const ROOT = document.querySelector('#section5');
if (!ROOT) throw new Error('Missing #section5');
...
const BLOOM_TRAIL_MAX_MS = 1000; // 1 second
let simSpeed = 1;
// ✅ Hard clear the trails canvas after fireworks finish (no physics/visual style change during the burst)
const HARD_CLEAR_AFTER_MS = 1000; // 1 second
...
function getMaskRect() {
  const w = MASK.w * stageW;
  const h = MASK.h * stageH;

  return {
    x: MASK.x * stageW,
    // ✅ y 从 bottom 计算：MASK.y = 距离底部的比例
    y: stageH - (MASK.y * stageH) - h,
    w,
    h
  };
}

function drawMaskDebug(ctx) {
  const r = getMaskRect();
  ctx.save();
...
function applyRectMaskCutout(ctx) {
  const r = getMaskRect();
  ctx.save();
...
function configDidUpdate() {
	...
	Spark.drawWidth = quality === QUALITY_HIGH ? 0.95 : 1.5;
}
...
Object.keys(appNodes).forEach(key => {
  appNodes[key] = $(appNodes[key]);
});


// Remove fullscreen control if not supported.
if (!fullscreenEnabled() && appNodes.fullscreenFormOption) {
...
function renderApp(state) {
	if (!appNodes.canvasContainer) return;
	const pauseBtnIcon = `#icon-${state.paused ? 'play' : 'pause'}`;
	const soundBtnIcon = `#icon-sound-${soundEnabledSelector() ? 'on' : 'off'}`;

	// ✅ 防止 controls 节点不存在导致脚本崩溃（不影响烟花渲染与 mask）
...
function getConfigFromDOM() {
  ...
  return {
    quality: appNodes.quality ? appNodes.quality.value : c.quality,
    shell: appNodes.shellType ? appNodes.shellType.value : c.shell,
    size: appNodes.shellSize ? appNodes.shellSize.value : c.size,
    autoLaunch: appNodes.autoLaunch ? appNodes.autoLaunch.checked : c.autoLaunch,
    finale: appNodes.finaleMode ? appNodes.finaleMode.checked : c.finale,
    skyLighting: appNodes.skyLighting ? appNodes.skyLighting.value : c.skyLighting,
    longExposure: appNodes.longExposure ? appNodes.longExposure.checked : c.longExposure,
    hideControls: appNodes.hideControls ? appNodes.hideControls.checked : c.hideControls,
    scaleFactor: appNodes.scaleFactor ? parseFloat(appNodes.scaleFactor.value) : c.scaleFactor
  };
};
...
function handleResize() {
  ...
  const w = Math.min(rect.width, MAX_WIDTH);
  const h = Math.min(rect.height, MAX_HEIGHT);

  appNodes.stageContainer.style.width = w + 'px';
  appNodes.stageContainer.style.height = h + 'px';
...
const io = new IntersectionObserver((entries) => {
  ...
  if (e.isIntersecting) {
    // Enter section5: allow sound + reset ramp
    ...
    // Leave section5: cancel pending start + pause + mute
    clearTimeout(startTimer);
    startTimer = null;
...
let autoLaunchTime = 9000;

//sssssssssssssss
// ---- Auto-launch ramp: first 6s slower, then very fast ----
let autoLaunchStartMs = 0;
const LAUNCH_RATE_FIRST6S = 1.25; //lower the more
const LAUNCH_RATE_AFTER6S = 0.6;
const LAUNCH_RATE_SWITCH_MS = 10000;

function resetAutoLaunchRamp() {
  autoLaunchStartMs = performance.now();
  autoLaunchTime = 9000;      // time delay
  isFirstSeq = true;          // optional: if you want “first sequence” behavior again
}
...
function updateGlobals(timeStep, lag) {
	...
	if (autoLaunchTime <= 0) {
		const elapsed = performance.now() - autoLaunchStartMs;
		const rate = elapsed < LAUNCH_RATE_SWITCH_MS ? LAUNCH_RATE_FIRST6S : LAUNCH_RATE_AFTER6S;
		autoLaunchTime = startSequence() * rate;
	}
	}
...
function update(frameTime, lag) {
  ...
  const timeStep = frameTime * simSpeed;     // physics step (affected by simSpeed)
  const speed = simSpeed * lag;             // rendering motion scaling
  ...
  // Track particles for optional debugging / (kept, but no longer required for fade)
  ...
  // Keep this if you want; not required for fade anymore, but harmless
  if (hasParticles) {
    lastParticleSeenAt = performance.now();
  }

  // ✅ Pass REAL frameTime (ms) into render so fading is time-based
  render(speed, frameTime);
}




function render(speed, frameTimeMs) {
  const { dpr } = mainStage;
  const width = stageW;
  const height = stageH;
...
// =======================================================
// Sky lighting (background tint behind particles)
// =======================================================
...
function colorSky(speed) {
  ...
  // When there are no stars, remove tint completely
  ...
  // Alpha scales with intensity (prevents “gray shadow” after)
  const a = 0.18 * intensity;

  appNodes.canvasContainer.style.backgroundColor =
    `rgba(${currentSkyColor.r | 0}, ${currentSkyColor.g | 0}, ${currentSkyColor.b | 0}, ${a})`;
}

// ✅ ticker must be OUTSIDE of colorSky, and only once
		...
		if (typeof this.color === 'string') {
			...
			if (this.ring) {
				...
				if (this.glitter) {
					star.sparkFreq = sparkFreq;
					star.sparkSpeed = sparkSpeed;
					star.sparkLife = Math.min(sparkLife, BLOOM_TRAIL_MAX_MS); // ✅ only cap spark
					star.sparkLifeVariation = sparkLifeVariation;
					star.sparkColor = this.glitterColor;
					star.sparkTimer = Math.random() * star.sparkFreq;
				}
...
// === Audio unlock (for autoplay restrictions) ===
function unlockAudioOnce() {
  // 这里只做“解锁 AudioContext”，不改变 soundEnabled
  if (soundManager && soundManager.ctx && soundManager.ctx.state !== 'running') {
